...
class DatabaseDataViewerScreen extends StatefulWidget {
  const DatabaseDataViewerScreen({Key? key}) : super(key: key);

  @override
  State<DatabaseDataViewerScreen> createState() =>
      _DatabaseDataViewerScreenState();
}

class _DatabaseDataViewerScreenState extends State<DatabaseDataViewerScreen>
  ...
  Future<void> _loadAllData() async {
    ...
    try {
      ...
      final results = await Future.wait([
        _localService.getShipments(limit: 1000), // Get all shipments
      ...
      for (final shipment in _shipments) {
        try {
          ...
          for (final box in boxes) {
            try {
              final products = await _localService.getProductsForBox(box.id);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Database Data Viewer'),
