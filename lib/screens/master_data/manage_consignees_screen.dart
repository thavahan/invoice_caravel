...
/// Screen for managing master consignees data
class ManageConsigneesScreen extends StatefulWidget {
  const ManageConsigneesScreen({Key? key}) : super(key: key);

  @override
  State<ManageConsigneesScreen> createState() => _ManageConsigneesScreenState();
}

class _ManageConsigneesScreenState extends State<ManageConsigneesScreen> {
  final DataService _dataService = DataService();
  final TextEditingController _searchController = TextEditingController();

  List<MasterConsignee> _consignees = [];
  List<MasterConsignee> _filteredConsignees = [];
  ...
  Future<void> _loadConsignees() async {
    if (!mounted) return;

    setState(() => _isLoading = true);
    print('🔄 MASTER_DATA_UI: Loading consignees...');

    try {
      final consigneesData = await _dataService.getMasterConsignees();
      final consignees =
          consigneesData.map((data) => MasterConsignee.fromMap(data)).toList();

      print('📊 MASTER_DATA_UI: Loaded ${consignees.length} consignees');

      if (mounted) {
        setState(() {
          _consignees = consignees;
          _filteredConsignees = consignees;
          _isLoading = false;
        });
      }
    } catch (e) {
      print('❌ MASTER_DATA_UI: Error loading consignees: $e');
      if (mounted) {
        setState(() => _isLoading = false);
        ScaffoldMessenger.of(context).hideCurrentSnackBar();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Error loading consignees: $e'),
  ...
  void _filterConsignees(String query) {
    setState(() {
      if (query.isEmpty) {
        _filteredConsignees = _consignees;
      } else {
        _filteredConsignees = _consignees
            .where((consignee) =>
                consignee.name.toLowerCase().contains(query.toLowerCase()) ||
                consignee.address.toLowerCase().contains(query.toLowerCase()))
            .toList();
      }
    });
  }
  ...
  Future<void> _showAddEditDialog({MasterConsignee? consignee}) async {
    final nameController = TextEditingController(text: consignee?.name ?? '');
    final phoneController = TextEditingController(text: consignee?.phone ?? '');
    final addr1Controller =
        TextEditingController(text: consignee?.addressLine1 ?? '');
    final addr2Controller =
        TextEditingController(text: consignee?.addressLine2 ?? '');
    final cityController = TextEditingController(text: consignee?.city ?? '');
    final stateController = TextEditingController(text: consignee?.state ?? '');
    final pincodeController =
        TextEditingController(text: consignee?.pincode ?? '');
    final landmarkController =
        TextEditingController(text: consignee?.landmark ?? '');

    // If editing and fields are empty, try to parse from stored address
    if (consignee != null &&
        phoneController.text.isEmpty &&
        addr1Controller.text.isEmpty) {
      final parsed = _parseAddress(consignee.address);
            ...
            Padding(
              ...
              child: Row(
                children: [
                  Expanded(
                    child: Text(
                      consignee == null ? 'Add Consignee' : 'Edit Consignee',
                      style:
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton(
                      ...
                      child: Text(consignee == null ? 'Add' : 'Update'),
                    ),
                  ),
                ],
              ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Manage Consignees'),
        ...
        children: [
          ...
          Padding(
            ...
            child: TextField(
              ...
              onChanged: _filterConsignees,
              decoration: InputDecoration(
                labelText: 'Search consignees...',
            ...
            child: _isLoading
                ...
                : _filteredConsignees.isEmpty
                          ...
                          children: [
                            ...
                            Text(
                              _searchController.text.isEmpty
                                  ? 'No consignees found\nTap + to add your first consignee'
                                  : 'No consignees match your search',
                    ...
                    : ListView.builder(
                        ...
                        itemCount: _filteredConsignees.length,
                        itemBuilder: (context, index) {
                          final consignee = _filteredConsignees[index];
                          return Card(
